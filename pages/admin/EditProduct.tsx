...
import { Save, Image, Loader2, ChevronRight, Sparkles, Trash2, Plus, Upload, Filter, X } from 'lucide-react';
import { db, storage } from '../../src/firebase';
import { collection, doc, getDoc, updateDoc, getDocs, serverTimestamp } from 'firebase/firestore';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { Category, Product, Brand, ShippingCompany } from '../../src/types';
...
const AdminEditProduct: React.FC = () => {
  ...
  const [digitalKeysInput, setDigitalKeysInput] = useState('');

  useEffect(() => {
    if (form.digitalKeys) {
      setDigitalKeysInput(form.digitalKeys.join('\n'));
    }
  }, [form.digitalKeys]);
  ...
  const removeImageField = (index: number) => {
    if ((form.images?.length || 0) <= 1) return;
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          console.error("Shipping companies fetch failed:", sErr);
        }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const keysArray = digitalKeysInput.split('\n').map(k => k.trim()).filter(Boolean);
    ...
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-2xl font-black text-white">تعديل المنتج</h2>
            ...
            <p className="text-sm opacity-50">{form.name_ar}</p>
          </div>
        </div>
      ...
      <div className={`bg-bg-card border ${form.isDigital ? 'border-purple-500/30' : isGamingCategory ? 'border-red-500/30' : 'border-white/10'} rounded-[2.5rem] p-8 shadow-2xl transition-all duration-500`}>
        <form onSubmit={handleSave} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                   ...
                   <input required type="number" className="w-full bg-bg-main border border-white/10 rounded-xl px-4 py-3 focus:border-primary outline-none transition-all" value={form.price || 0} onChange={e => setForm({...form, price: Number(e.target.value)})} placeholder="0" />
                </div>
                <div className="space-y-2">
                   <label className="text-xs opacity-40">سعر الشراء جملة (IQD)</label>
                   <input type="number" className="w-full bg-bg-main border border-white/10 rounded-xl px-4 py-3 focus:border-primary outline-none transition-all" value={form.costPrice || 0} onChange={e => setForm({...form, costPrice: Number(e.target.value)})} placeholder="0" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                   ...
                   <input type="number" className="w-full bg-bg-main border border-white/10 rounded-xl px-4 py-3 focus:border-primary outline-none transition-all" value={form.stockQuantity || 0} onChange={e => setForm({...form, stockQuantity: Number(e.target.value)})} placeholder="0" />
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                   ...
                   <div className="flex items-center gap-4 p-4 bg-bg-main border border-blue-500/30 rounded-xl">
                      ...
                      />
                      <span className="text-sm font-bold text-blue-400">نشر تحديث في التليكرام؟</span>
                   </div>
                </div>
              </div>
            </div>
            ...
            {/* Section: Digital Content (Conditional) */}
            {form.isDigital && (
              <div className="space-y-6 p-6 bg-purple-500/5 border border-purple-500/20 rounded-3xl animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="flex items-center gap-3 border-b border-purple-500/10 pb-4">
                  <div className="w-8 h-8 rounded-lg bg-purple-500/20 text-purple-400 flex items-center justify-center">
                    <Plus className="w-4 h-4" />
                  </div>
                  <h3 className="text-lg font-black text-white uppercase italic tracking-tighter">محتوى المنتج الرقمي</h3>
                </div>

                <div className="space-y-6">
                  <div className="space-y-2">
                     <label className="text-xs opacity-40">تعليمات الاستلام / رابط التحميل</label>
                     <textarea 
                       className="w-full bg-bg-main border border-white/10 rounded-xl px-4 py-3 h-24 focus:border-purple-500 outline-none transition-all" 
                  ...
                  <div className="space-y-2">
                     <div className="flex justify-between items-center">
                       ...
                       <span className="text-[10px] text-purple-400 font-bold bg-purple-500/10 px-2 py-1 rounded-full">
                         {digitalKeysInput.split('\n').filter(k => k.trim()).length} كود متوفر
                       </span>
                     </div>
                     <textarea 
                       className="w-full bg-bg-main border border-white/10 rounded-xl px-4 py-3 h-32 font-mono text-sm focus:border-purple-500 outline-none transition-all" 
                       value={digitalKeysInput} 
                       onChange={e => setDigitalKeysInput(e.target.value)} 
            ...
            <div className="flex items-center gap-4 pt-8 border-t border-white/5">
              <button type="submit" disabled={submitting} className={`flex-grow py-4 ${isGamingCategory ? 'bg-red-600 hover:bg-red-700 shadow-red-600/20' : 'bg-primary hover:bg-primary-dark shadow-primary/20'} text-white font-black rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2`}>
                {submitting ? <Loader2 className="animate-spin" /> : <><Save className="w-5 h-5" /> حفظ التغييرات</>}
              </button>
